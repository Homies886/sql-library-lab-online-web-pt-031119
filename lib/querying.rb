...
def select_series_title_with_most_human_characters
  "select series.title from series join books on series.book_id = books.id join character_books on books.id = character_books.book_id join characters on character_books.character_id = characters.id where character.species = 'human' group by series.title order by count(*) desc limit 1;"
end
